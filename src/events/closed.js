  ...
  app.post("/closed", async (req, res) => {
    try {
      const mutation = await authenticatedGraphQl(ADD_COMMENT, {
        ...
        body: `This contract at ${process.env.BASE_URL}/contract/${req.body.bountyId}/${req.body.id} has been claimed by the author of this pull request: ${req.body.closerData}.`,
      });
      res.json(mutation);
    } catch (err) {
      console.error(err);
